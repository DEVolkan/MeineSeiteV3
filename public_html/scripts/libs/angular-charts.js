    ...
    function($templateCache, $compile, $rootElement, $window, $timeout, $sce) {
        ...
        function link(scope, element, attrs) {
            ...
            function barChart() {
                ...
                filterXAxis(xAxis, x);
                var yAxis = d3.svg.axis().scale(y).orient('left').ticks(10).tickFormat(d3.format('d'));
                /**
        ...
        svg.append('g').attr('class', 'y axis').call(yAxis);
                ...
                path.append('path').style('fill', function(d, i) {
                    ...
                    if (!complete) {
                        ...
                        path.on('mouseover', function(d) {
                            ...
                            d3.select(this).select('path').transition().duration(200).style('stroke', 'white').style('stroke-width', '4px');
                            config.mouseover(d, d3.event);
                            scope.$apply();
                        }).on('mouseleave', function(d) {
            ...
            function pointChart() {
                ...
                var xAxis = d3.svg.axis().scale(x).orient('bottom');
                filterXAxis(xAxis, x);
                var yAxis = d3.svg.axis().scale(y).orient('left').ticks(5).tickFormat(d3.format(''));
                var yData = [0];
                var linedata = [];
                points.forEach(function(d) {
            ...
            function makeToolTip(data, event) {
                ...
                el.css.left = event.pageX + 20 + 'px';
                el.css.top = event.pageY - 30 + 'px';
